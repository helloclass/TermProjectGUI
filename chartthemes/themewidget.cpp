...
ThemeWidget::ThemeWidget(QWidget *parent) :
    ...
    m_ui(new Ui_ThemeWidgetForm)
    {
        ...
        chartView = new QChartView(createHBarChart(m_valueCount));
...
QChart *ThemeWidget::createAreaChart() const
{
    ...
    for (int i(0); i < m_dataTable.count(); i++) {
        ...
        for (int j(0); j < m_dataTable[i].count(); j++) {
            ...
            // confirm lowerSeries(previous data) because have to accumulate data.
            if (lowerSeries) {
                // set position to data Qpair in LowerSeries
...
QChart *ThemeWidget::createHBarChart(int valueCount) const
{
    ...
    chart->createDefaultAxes();
    chart->axes(Qt::Horizontal).first()->setRange(0, m_valueMax * 2);

    // Add space to label to add space between labels and axis
    QValueAxis *axisY = qobject_cast<QValueAxis*>(chart->axes(Qt::Horizontal).first());
    Q_ASSERT(axisY);
    axisY->setLabelFormat("%.1f  ");
